  ...
  function resolveBaseUrl(): string {
    // 运行时可由外部在 window 注入
    // @ts-ignore
    const runtime = (typeof window !== 'undefined' && (window.API_BASE_URL || window.__API_BASE_URL__)) || '';
    ...
    // Node 环境变量（打包时注入）
    // @ts-ignore
    const nodeEnv = (typeof process !== 'undefined' && (process.env.API_BASE_URL || process.env.VITE_API_BASE)) || '';
  ...
  // ===================== 全局消息/弹窗/通知 =====================
  /** 全局消息提示 */
  const showInfo = (title: string, duration = 2000) => Taro.showToast({ title, icon: 'none', duration })
  /** 全局错误提示 */
  const showError = (title: string, duration = 2000) => Taro.showToast({ title, icon: 'error', duration })
  /** 全局加载提示 */
  const showLoading = (title = '加载中...') => Taro.showLoading({ title, mask: true })
  /** 全局通知（统一为无取消的 Modal） */
  const showNotification = (title: string, content?: string) => Taro.showModal({ title, content: content || '', showCancel: false })
  /** 全局确认弹窗 */
  const showConfirm = (title: string, onOk?: () => void, onCancel?: () => void, content?: string) => {
  ...
  // ===================== 全局页面跳转/导航 =====================
  /** Umi4 路由跳转（推荐） */
  function goto(url: string, type: 'navigateTo' | 'redirectTo' | 'reLaunch' | 'switchTab' = 'navigateTo') {
    switch (type) {
      case 'navigateTo':
